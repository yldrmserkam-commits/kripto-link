...
def analyze_symbol(symbol, key, info, sent_signals):
    ...
    if cci_cross_above_minus100:
        # TradingView linkinde BINANCE: öneki kaldırıldı, doğrudan sembol yazılıyor
        clean_symbol = symbol if "USDT" in symbol else f"{symbol}USDT"
        tv_link = f"https://www.tradingview.com/chart/?symbol={clean_symbol}"
