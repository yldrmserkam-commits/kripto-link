...
MAX_WORKERS = 20  # Paralel işlem kapasitesi artırıldı
CCI_PERIYOT = 20  # Standart CCI periyodu
MAX_COIN_LIMIT = 120  # Sadece en yüksek hacimli ilk 120 coin taranacak
...
def get_top_volume_symbols(limit=MAX_COIN_LIMIT):
    """Binance Futures 24 saatlik hacim verilerine göre en aktif coinleri seçer."""
    try:
        ...
        if res.status_code == 200:
            ...
            for item in data:
                sym = item.get('symbol', '')
                if sym.endswith('USDT') and not sym.endswith('_UPUSDT') and not sym.endswith('_DOWNUSDT'):
                    try:
                        q_vol = float(item.get('quoteVolume', 0))
                        usdt_pairs.append((sym, q_vol))
                    except:
                        pass
            
            # Hacme göre büyükten küçüğe sırala
            usdt_pairs.sort(key=lambda x: x[1], reverse=True)
            top_symbols = [p[0] for p in usdt_pairs[:limit]]
            if top_symbols:
                print(f"🎯 Hacim filtresi aktif: En yüksek hacimli {len(top_symbols)} coin seçildi.")
                return top_symbols
    except Exception as e:
        print(f"Hacim listesi alınamadı, yedek yönteme geçiliyor: {e}")
...
if __name__ == "__main__":
    print(f"🔄 Paralel Tarama Başladı: {datetime.now()}")
    ...
    if not symbols:
        print("⚠️ Hacim listesi alınamadı, işlem durduruldu.")
        exit()

    active_periods = {k: v for k, v in TARAMA_PERIYOTLARI.items() if v}
    
    # Tüm coin ve periyot kombinasyonlarını tek bir görev listesine (task list) topla
    tasks = []
    for sym in symbols:
        for key, info in active_periods.items():
            tasks.append((sym, key, info))

    print(f"⚡ Toplam {len(symbols)} coin ve {len(active_periods)} periyot ({len(tasks)} görev) eş zamanlı taranıyor...")
    ...
    # Tek bir ThreadPool ile hem coinler hem periyotlar aynı anda taranır
